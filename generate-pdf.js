function exportToPDF() {
  ...
  setTimeout(() => {
    ...
    const opt = {
      margin: [0.8, 0.8],
      filename: `preventivo_${localStorage.getItem("numeroPreventivo")}.pdf`,
      html2canvas: {
        scale: 1,
        scrollY: false,
        ignoreElements: (element) => element.classList.contains('no-print'),
        // Configurazione ottimizzata per html2canvas
        useCORS: true,
        logging: false,
        allowTaint: true
      },
      ...
      // Configurazione aggiuntiva per il rendering
      jsPDF: {
        unit: 'mm',
        format: 'a4',
    ...
    html2pdf()
      ...
      .then(() => {
        ...
        return incrementaNumeroPreventivo();
      })
      .catch((error) => {
...
// Funzioni di supporto per la numerazione (rimangono identiche)
