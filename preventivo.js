...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Observer per sincronizzazione automatica
  const observer = new MutationObserver((mutations) => {
      secondario.textContent = mutations[0].target.textContent;
  });

  // Funzione unica per aggiornamento numerazione
  const aggiornaNumerazione = (numero) => {
...
function isModuleActive(moduleTableId) {
  ...
  // Funzione di sincronizzazione globale
function sincronizzaNumeriPreventivo() {
  const numeroOriginale = document.getElementById("numeroPreventivo");
  const numeroCopia = document.getElementById("numeroPreventivoSecondario");
  
  if(numeroOriginale && numeroCopia) {
    numeroCopia.textContent = numeroOriginale.textContent;
  }
}