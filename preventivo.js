document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelectorAll(".module-table tbody").forEach(tbody => {
        tbody.innerHTML = ""; // Svuota la tabella iniziale
    });
});
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function isModuleActive(moduleTableId) {
      ...
      for (let input of inputs) {
        if (input.value.trim() !== "" && input.value.trim() !== "0") return true;
      }
      return false;
    }
  ...
  // Numero automatico
  // Recupera il numero progressivo dal server
async function getNumeroPreventivo() {
    let response = await fetch("http://localhost:3000/numero");
    let data = await response.json();
    document.getElementById("numeroPreventivo").innerText = data.numeroPreventivo;
}

// Incrementa il numero quando si esporta il PDF
async function esportaPDF() {
    let response = await fetch("http://localhost:3000/incrementa", { method: "POST" });
    let data = await response.json();
    
    document.getElementById("numeroPreventivo").innerText = data.numeroPreventivo;
}

// Quando la pagina si carica, mostra il numero preventivo
getNumeroPreventivo();

// Collega il pulsante all'export
document.getElementById("exportBtn").addEventListener("click", esportaPDF);
