...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const exportButton = document.getElementById("exportPDF"); // Assicurati di avere l'ID giusto

  // Funzione per ottenere il numero progressivo dal server
  function fetchNumeroPreventivo() {
      fetch("http://localhost:3000/numero")
          .then(response => response.json())
          .then(data => {
              numeroPreventivo.textContent = data.numeroPreventivo;
              localStorage.setItem("numeroPreventivo", data.numeroPreventivo); // Aggiorna anche localStorage
          })
          .catch(error => console.error("Errore nel recupero del numero:", error));
  }

  // Funzione per incrementare il numero preventivo dopo l'esportazione
  function incrementaNumeroPreventivo() {
      fetch("http://localhost:3000/incrementa", { method: "POST" })
          .then(response => response.json())
          .then(data => {
              numeroPreventivo.textContent = data.numeroPreventivo;
              localStorage.setItem("numeroPreventivo", data.numeroPreventivo);
          })
          .catch(error => console.error("Errore nell'incremento del numero:", error));
  }

  // Quando la pagina si carica, recupera il numero attuale
  fetchNumeroPreventivo();

  // Incrementa il numero quando si clicca il bottone di esportazione PDF
  exportButton.addEventListener("click", function () {
      setTimeout(() => { // Attendi un momento per evitare problemi di sincronizzazione
...
function exportToPDF() {
  // Nascondi il bottone per non includerlo nel PDF
  const btn = document.querySelector('button[onclick="exportToPDF()"]');
  if (btn) btn.style.display = 'none';

  // Seleziona l'elemento che contiene tutto il preventivo
  ...
  // Clona l'intero elemento per evitare problemi di overflow
  const clone = element.cloneNode(true);
  clone.style.overflow = 'visible';
  clone.style.height = 'auto';

  // Crea un contenitore temporaneo off-screen
  ...
  // Attendi un breve delay per assicurarti che il clone sia renderizzato
  setTimeout(() => {
    const opt = {
      margin: 0.5, // margine in cm
      filename: `preventivo_${document.getElementById("numeroPreventivo").textContent || "default"}.pdf`,
      image: { type: 'png', quality: 1 },
      html2canvas: {
        scale: 1,                  // Scala reale
        useCORS: true,             // Abilita CORS per le immagini con crossorigin="anonymous"
        allowTaint: false,
        backgroundColor: '#ffffff',
        scrollY: 0,                // Nessun offset verticale
        height: clone.scrollHeight,    // Altezza totale del clone
        windowWidth: clone.scrollWidth   // Larghezza totale del clone
      },
      jsPDF: { unit: 'cm', format: 'a4', orientation: 'portrait' }
    };

    // Genera il PDF usando html2pdf
    html2pdf().set(opt).from(clone).save()
      ...
      .finally(() => {
        // Rimuovi il contenitore temporaneo e ripristina il bottone
